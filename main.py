...
LINK_VOTE = "https://top.gg/bot/1012217155141517312/vote"
...
async def get_user(request):
    ...
    if DATA is not None:
        ...
        NAME, DESC, AVATAR = USER.name, USER.discriminator, USER.avatar.url
...
async def top_gg(request):
    ...
    if _AUTH_POST == DBL_AUTH:
        ...
        AVATAR = USER.avatar.url if USER is not None else None
        THUMBNAIL = AVATAR if AVATAR is not None else ASH.avatar.url
...
if __name__ == "__main__":
    web.run_app(main_server(), host=HOST, port=PORT)
